angular.module('hairdresser.settings', ['config', 'hairdresser.settings.social', 'security.service', 'security.authorization', 'services.hairdresserResource', 'services.utility','ui.bootstrap', 'directives.serverError']);
angular.module('hairdresser.settings').config(['$routeProvider', 'securityAuthorizationProvider', function($routeProvider){
  $routeProvider
    .when('/hairdresser/settings', {
      templateUrl: 'hairdresser/settings/hairdresser-settings.tpl.html',
      controller: 'HairdresserSettingsCtrl',
      title: 'Account Settings',
      resolve: {
        hairdresserDetails: ['$q', '$location', 'securityAuthorization', 'hairdresserResource' ,function($q, $location, securityAuthorization, hairdresserResource){
          //get hairdresser details only for verified-user, otherwise redirect to /hairdresser/verification
          var redirectUrl;
...
angular.module('hairdresser.settings').controller('HairdresserSettingsCtrl', [ '$scope', '$location', '$log', 'security', 'utility', 'hairdresserResource', 'hairdresserDetails', 'SOCIAL',
  function($scope, $location, $log, security, utility, restResource, hairdresserDetails, SOCIAL){
    //local vars
    var hairdresser = hairdresserDetails.hairdresser;
    var user = hairdresserDetails.user;
    var submitDetailForm = function(){
      ...
      restResource.setAccountDetails($scope.userDetail).then(function(data){
        ...
        $scope.alerts.detail.push({
          type: 'danger',
          msg: 'Error updating hairdresser details: ' + x
        });
      });
    };
    ...
    var disconnect = function(provider){
      var errorAlert = {
        type: 'warning',
        msg: 'Error occurred when disconnecting your '+ provider + ' hairdresser. Please try again later.'
      };
      $scope.socialAlerts = [];
      security.socialDisconnect(provider).then(function(data){
        if(data.success){
          ...
          $scope.socialAlerts.push({
            type: 'info',
            msg: 'Successfully disconnected your '+ provider +' hairdresser.'
          });
        }else{
    ...
    $scope.userDetail = {
      first:  hairdresser.name.first,
      middle: hairdresser.name.middle,
      last:   hairdresser.name.last,
      company:hairdresser.company,
    ...
    if(search.provider){
      if(search.success === 'true'){
        $scope.socialAlerts.push({
          type: 'info',
          msg: 'Successfully connected your '+ search.provider +' hairdresser.'
        });
      }else{
        $scope.socialAlerts.push({
          type: 'warning',
          msg: 'Unable to connect your '+ search.provider + ' hairdresser. ' + search.reason
        });
      }
    }
