...
var hairdresser = {
  find: function (req, res, next) {
    ...
    var getStatusOptions = function (callback) {
      req.app.db.models.Status.find({pivot: 'Hairdresser'}, 'name').sort('name').exec(function (err, statuses) {
        if (err) {
  ...
  create: function(req, res, next){
    ...
    workflow.on('createHairdresser', function() {
      ...
      req.app.db.models.Hairdresser.create(fieldsToSet, function(err, hairdresser) {
        ...
        workflow.outcome.record = hairdresser;
        return workflow.emit('response');
      });
    });
  ...
  read: function(req, res, next){
    ...
    var getRecord = function(callback) {
      req.app.db.models.Hairdresser.findById(req.params.id).exec(function(err, record) {
        if (err) {
          return callback(err, null);
        }
  ...
  update: function(req, res, next){
    ...
    workflow.on('patchHairdresser', function() {
      ...
      req.app.db.models.Hairdresser.findByIdAndUpdate(req.params.id, fieldsToSet, options, function(err, hairdresser) {
        if (err) {
          return workflow.emit('exception', err);
        }
  ...
  linkUser: function(req, res, next){
    ...
    workflow.on('verifyUser', function(callback) {
      req.app.db.models.User.findOne({ username: req.body.newUsername }).exec(function(err, user) {
        ...
        else if (user.roles && user.roles.hairdresser && user.roles.hairdresser !== req.params.id) {
          workflow.outcome.errors.push('User is already linked to a different hairdresser.');
          return workflow.emit('response');
        }
    ...
    workflow.on('duplicateLinkCheck', function(callback) {
      req.app.db.models.Hairdresser.findOne({ 'user.id': workflow.user._id, _id: {$ne: req.params.id} }).exec(function(err, account) {
        if (err) {
          return workflow.emit('exception', err);
        }
    ...
    workflow.on('patchUser', function() {
      req.app.db.models.User.findByIdAndUpdate(workflow.user._id, { 'roles.hairdresser': req.params.id }).exec(function(err, user) {
        if (err) {
          return workflow.emit('exception', err);
        }
    ...
    workflow.on('patchAccount', function(callback) {
      req.app.db.models.Hairdresser.findByIdAndUpdate(req.params.id, { user: { id: workflow.user._id, name: workflow.user.username } }, { new: true }).exec(function(err, account) {
        if (err) {
          return workflow.emit('exception', err);
        }
  ...
  unlinkUser: function(req, res, next){
    ...
    workflow.on('patchAccount', function() {
      req.app.db.models.Hairdresser.findById(req.params.id).exec(function(err, hairdresser) {
        if (err) {
          return workflow.emit('exception', err);
        }
        ...
        var userId = hairdresser.user.id;
        hairdresser.user = { id: undefined, name: '' };
    ...
    workflow.on('patchUser', function(id) {
      req.app.db.models.User.findById(id).exec(function(err, user) {
        ...
        user.roles.hairdresser = undefined;
  ...
  newNote: function(req, res, next){
    ...
    workflow.on('addNote', function() {
      ...
      var options = { new: true };
      req.app.db.models.Hairdresser.findByIdAndUpdate(req.params.id, { $push: { notes: noteToAdd } }, options, function(err, hairdresser) {
        if (err) {
          return workflow.emit('exception', err);
        }
  ...
  newStatus: function(req, res, next){
    ...
    workflow.on('addStatus', function() {
      ...
      req.app.db.models.Hairdresser.findByIdAndUpdate(req.params.id, { status: statusToAdd, $push: { statusLog: statusToAdd } }, options, function(err, hairdresser) {
        if (err) {
          return workflow.emit('exception', err);
        }
  ...
  delete: function(req, res, next){
    ...
    workflow.on('validate', function() {
      ...
      workflow.emit('deleteHairdresser');
    });

    workflow.on('deleteHairdresser', function(err) {
      req.app.db.models.Hairdresser.findByIdAndRemove(req.params.id, function(err, hairdresser) {
        if (err) {
          return workflow.emit('exception', err);
        }
